...
Crabby 内置了以下工具，LLM 可以自主调用：

| 工具 | 说明 | 示例 |
|------|------|------|
| `shell` | 执行 PowerShell 命令 | 查看磁盘空间、运行脚本等 |
| `file_read` | 读取文件内容 | 读取配置文件、日志等 |
| `file_write` | 写入或追加文件 | 创建文件、修改配置等 |
| `file_list` | 列出目录内容 | 浏览文件夹 |
| `web_fetch` | 抓取网页内容 | 读取网页信息 |
| `web_search` | 搜索网页（DuckDuckGo） | 查询实时信息 |
| `memory_save` | 保存信息到持久记忆 | 记住重要事项 |
| `skill_run` | 运行已安装的技能 | 调用自定义技能 |
...
- Shell 工具具有完整的系统访问权限，请注意不要让 LLM 执行危险命令
